...
const SeriesCard = ({ series, coverImage }) => {
  ...
  const deleteSeries = async () => {
    setIsDeleting(true);
    const deleteResp = await axios.post(`/api/sundae/series/${series.id}/delete`);
    setIsDeleting(false);
    toggleDeleteModal();
  ...
  return (
    ...
    >
      ...
      <div
        // onClick={() => router.push(`/series/${series.id}/episodes?series=${series.title}`)}
        // onClick={() => router.push(`/series/${series.id}`)}
        onClick={() => router.push({pathname: `/series/${series.id}`, query: { title: series.title }})}
      >
        <div className={styles.img_container}>
          <img src={coverImage.srcUrl} alt={series.title} />
        </div>
